        ...
        <div className="bg-white rounded-[2.5rem] shadow-2xl border border-slate-100 p-10 relative">
          ...
          <button
            onClick={async () => {
              ...
              // If no error, Google redirects automatically — no need to reset
            }}
            disabled={loading || signingIn}
            className="w-full flex items-center justify-center gap-3 h-14 px-6 bg-white border-2 border-slate-200 hover:border-[#5630D1]/30 hover:bg-slate-50 rounded-2xl font-bold text-slate-700 text-base transition-all shadow-sm hover:shadow-md active:scale-95 group disabled:opacity-60 disabled:cursor-not-allowed"
          >
            {signingIn ? (
              <span className="material-symbols-outlined animate-spin text-[22px] text-[#5630D1]">progress_activity</span>
            ) : (
